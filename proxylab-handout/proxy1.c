...
int connect_server(char *hostname,int port,char *query_path)
{
    ...
    Rio_writen(proxy_clientfd,(void*)buf,strlen(buf));
    sprintf(buf,"Host: %s\r\n",hostname);
    Rio_writen(proxy_clientfd,(void*)buf,strlen(buf));
    Rio_writen(proxy_clientfd,(void*)user_agent,strlen(user_agent));
    Rio_writen(proxy_clientfd,(void*)accept_str,strlen(accept_str));
    Rio_writen(proxy_clientfd,(void*)connection,strlen(connection));
